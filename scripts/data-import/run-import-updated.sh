...
# This script processes UFO intelligence markdown files, sightings data, and testimonies and imports them to Xata
...
# Source the testimony import function
source "$BASE_DIR/testimony-import-function.sh"
...
echo "3. Import UFO Testimonies data"
echo "4. Import all data types"
echo "q. Quit"
echo
read -p "Enter your choice (1, 2, 3, 4, or q): " choice
...
case $choice in
  ...
  2)
    ...
    ;;
  3)
    # Import testimonies only
    import_events=false
    import_sightings=false
    import_testimonies=true
    ;;
  4)
    # Import all
    import_events=true
...
# Import testimonies if selected
if [ "$import_testimonies" = true ]; then
  import_testimonies
  testimonies_result=$?
else
  testimonies_result=0
fi

# Final status
if [ $events_result -eq 0 ] && [ $sightings_result -eq 0 ] && [ $testimonies_result -eq 0 ]; then
