...
import {RAGMention} from '@/components/tiptap/tiptap-extension/rag/rag-mention'
...
import {RAGDropdown} from '@/components/tiptap/tiptap-ui/rag-dropdown'
import {TextAlignButton} from '@/components/tiptap/tiptap-ui/text-align-button'
import type {UndoRedoButton} from '@/components/tiptap/tiptap-ui/undo-redo-button'
...
interface RAGEnhancedEditorProps {
  content?: any
  onContentChange?: (content: any) => void
}

export function RAGEnhancedEditor({
  content,
  onContentChange,
}: RAGEnhancedEditorProps) {
  ...
  const ragHandler = React.useMemo(() => new RAGLLMHandler(), [])
  ...
  const editor = useEditor({
    ...
    extensions: [
      ...
      // Add RAG Mention extension when enabled
      ...(ragEnabled
        ? [
            RAGMention.configure({
              ragHandler,
              HTMLAttributes: {
                class: 'rag-mention',
              },
    ...
    <EditorContext.Provider value={{editor}}>
      ...
      {ragEnabled && (
        <div className='text-xs text-muted-foreground mt-2'>
          Tip: Type @@ to search the knowledge base
        </div>
      )}
