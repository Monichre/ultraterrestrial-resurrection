...
class KnowledgeBaseIndexer:
    ...
    def index_case_files(self) -> List[Dict[str, Any]]:
        """Index PDF case files"""
        indexed = []
        
        if not self.case_files_path.exists():
            logger.warning(f"Case files directory not found: {self.case_files_path}")
            return indexed
    ...
    def index_transcripts(self) -> List[Dict[str, Any]]:
        ...
        if not self.transcripts_path.exists():
            logger.warning(f"Transcripts directory not found: {self.transcripts_path}")
            return indexed
    ...
    def index_research(self) -> List[Dict[str, Any]]:
        ...
        if not self.research_path.exists():
            logger.warning(f"Research directory not found: {self.research_path}")
            return indexed
    ...
    def build_index(self) -> bool:
        ...
        all_documents = []
        all_documents.extend(self.index_case_files())
