# DeepKnowledge - AI Agent

- ID: 6XUGfVbN5AxhNIdLDDQiQz
- Name: DeepKnowledge - AI Agent
- Type: Notebook

Summary
- Iteratively searches a knowledge base (Agno docs example), decomposes queries, performs multiple searches, documents reasoning and citations, then synthesizes a comprehensive answer.
- Uses: UrlKnowledge + LanceDb (hybrid search), OpenAI embeddings, SqliteAgentStorage

Original Notebook Content (verbatim, normalized)

````text
#  DeepKnowledge - AI Agent 
An Agent that iteratively searches a knowledge base to answer questions
...
def create_agent(session_id: Optional[str] = None) -> Agent:
    ...
    return Agent(
        ...
        description=dedent("""You are DeepKnowledge, an advanced reasoning agent designed to provide thorough,
...
def run_interactive_loop(agent: Agent):
    ...
    while True:
        ...
        questions = [inquirer.List("topic", message="Select a topic or ask a different question:", choices=choices)]
