...
class UAPKnowledgeGraphConfig:
    ...
    def _load_flow_config(self) -> FlowConfig:
        ...
        return FlowConfig(
            ...
            enable_neo4j=os.getenv(
                "COCOINDEX_ENABLE_NEO4J", "true").lower() == "true",
    ...
    def get_ner_prompt_path(self) -> Path:
        """Get path to canonical NER prompt YAML in packages/prompts."""
        repo_root = self.project_root.parent.parent
        return repo_root / "packages" / "prompts" / "sets" / "disclosure" / "ner.v1.yaml"
    ...
    def validate(self) -> Dict[str, Any]:
        ...
        if self.flow.enable_neo4j and not self.neo4j:
            status["warnings"].append(
                "Neo4j enabled but configuration missing")

        # Check file paths
        if not self.get_ner_prompt_path().exists():
            status["warnings"].append(
                f"NER prompt file not found: {self.get_ner_prompt_path()}")

        if not self.get_schema_sql_path().exists():
            status["warnings"].append(
                f"Schema SQL file not found: {self.get_schema_sql_path()}")
