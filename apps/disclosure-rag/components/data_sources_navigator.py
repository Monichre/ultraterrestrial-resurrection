...
class DataSourcesNavigator:
    ...
    def _scan_with_index(self) -> Dict[str, Any]:
        ...
        try:
            ...
            data_sources = {
                'case_files': {'files': [], 'count': 0, 'total_size_mb': 0, 'categories': Counter()},
            ...
            for doc_id, doc_info in index_data.get('documents', {}).items():
                ...
                if doc_type == 'case_file':
                    file_info['size_mb'] = doc_info['metadata'].get('file_size', 0) / (1024 * 1024) if doc_info['metadata'].get('file_size') else 0
                    file_info['category'] = self._categorize_case_file(doc_info['title'])
                    data_sources['case_files']['files'].append(file_info)
                    data_sources['case_files']['count'] += 1
                    data_sources['case_files']['total_size_mb'] += file_info['size_mb']
                    data_sources['case_files']['categories'][file_info['category']] += 1
                
                elif doc_type == 'transcript':
                    file_info['size_kb'] = doc_info['metadata'].get('file_size', 0) / 1024 if doc_info['metadata'].get('file_size') else 0
                    file_info['topic'] = self._extract_transcript_topic(doc_info['title'])
                    file_info['date_folder'] = doc_info['metadata'].get('date_folder', 'unknown')
                    ...
                    if file_info['date_folder'] not in data_sources['transcripts']['date_folders']:
                        data_sources['transcripts']['date_folders'].append(file_info['date_folder'])
    ...
    def _scan_case_files(self) -> Dict[str, Any]:
        """Scan PDF case files"""
        case_files = []
        
        if self.case_files_path.exists():
            for file_path in self.case_files_path.glob("**/*.pdf"):
                try:
                    stat = file_path.stat()
                    case_files.append({
                        'name': file_path.name,
        ...
        return {
            ...
            'total_size_mb': sum(f['size_mb'] for f in case_files),
            'categories': Counter(f['category'] for f in case_files)
        }
    ...
    def _generate_summary(self, data_sources: Dict[str, Any]) -> Dict[str, Any]:
        ...
        total_size_mb = (
            data_sources['case_files']['total_size_mb'] +
        ...
        return {
            ...
            'file_types': {
                'Case Files (PDFs)': data_sources['case_files']['count'],
    ...
    def render_overview_dashboard(self, data_sources: Dict[str, Any]):
        ...
        with col3:
            ...
            if date_range['earliest'] and date_range['latest']:
                ...
                st.metric("Date Coverage", "No data", help="No transcript dates found")
    ...
    def render_detailed_browser(self, data_sources: Dict[str, Any]):
        ...
        # Category selector
        categories = ['All', 'Case Files', 'Transcripts', 'Articles', 'Research']
        selected_category = st.selectbox("Select Category", categories)
        
        # Search functionality
        search_term = st.text_input("🔍 Search files", placeholder="Enter filename or content keywords...")
        ...
        filtered_files = self._filter_files(data_sources, selected_category, search_term)
        ...
        if filtered_files:
            ...
            display_columns = ['name', 'category', 'type', 'size_display', 'modified']
            column_config = {
    ...
    def _filter_files(self, data_sources: Dict[str, Any], category: str, search_term: str) -> List[Dict]:
        ...
        if category == 'All':
            categories_to_include = ['case_files', 'transcripts', 'articles', 'research']
        elif category == 'Case Files':
            categories_to_include = ['case_files']
            ...
            try:
                ...
                with st.expander("📋 Document Metadata"):
                    ...
                    with col2:
                        ...
                        st.write(f"**Tags:** {', '.join(file_info['tags']) if file_info['tags'] else 'None'}")
                ...
                if file_path.exists():
                    ...
                    if file_info['type'] == 'PDF':
                        st.info("PDF content extraction not yet implemented. Showing file information only.")
    ...
    def render_statistics_panel(self, data_sources: Dict[str, Any]):
        ...
        categories = ['case_files', 'transcripts', 'articles', 'research']
            ...
            with st.expander(f"📁 {category.replace('_', ' ').title()} Details"):
                ...
                with col2:
                    if 'total_size_mb' in cat_data:
                        st.metric("Total Size", f"{cat_data['total_size_mb']} MB")
                    elif 'total_size_kb' in cat_data:
                        st.metric("Total Size", f"{cat_data['total_size_kb']} KB")
                
                with col3:
                    if category == 'case_files' and 'categories' in cat_data:
    ...
    def render_main_interface(self):
        ...
        if st.session_state.last_scan_time:
            st.caption(f"Last scanned: {st.session_state.last_scan_time.strftime('%Y-%m-%d %H:%M:%S')}")
        
        # Tab interface
        if HAS_DATABASE_STATS:
            tab1, tab2, tab3, tab4 = st.tabs(["📊 Overview", "🔍 File Browser", "📈 Statistics", "🗄️ Database"])
        else:
            tab1, tab2, tab3 = st.tabs(["📊 Overview", "🔍 File Browser", "📈 Statistics"])
