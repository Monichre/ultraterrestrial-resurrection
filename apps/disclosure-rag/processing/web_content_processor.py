...
class WebContentProcessor:
    ...
    def setup_storage_directories(self):
        ...
        self.case_files_dir = self.knowledge_dir / "case_files"
        self.images_dir = self.case_files_dir / "images"
        self.pdfs_dir = self.case_files_dir / "pdfs"
        
        # Create directories if they don't exist
        for directory in [self.knowledge_dir, self.case_files_dir, self.images_dir, self.pdfs_dir]:
        ...
        try:
            with Image.open(filepath) as img:
                ...
                if hasattr(img, '_getexif') and img._getexif():
                    exif = img._getexif()
                    info['exif'] = {k: v for k, v in exif.items() if isinstance(v, (str, int, float))}
    ...
    def analyze_image_content(self, thumbnail_b64: str, url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            context_analysis = analysis_engine.analyze_content(image_analysis_prompt)
    ...
    def handle_pdf(self, url: str) -> Dict[str, Any]:
        try:
            ...
            pdf_analysis = analysis_engine.analyze_content(markdown_content[:10000])  # Limit for analysis
    ...
    def scrape_url(self, url: str) -> Dict[str, Any]:
        try:
            ...
            if self.enable_media_extraction:
                ...
                for pdf_url in extracted_links.get('pdfs', [])[:5]:  # Limit PDF downloads
                    try:
    ...
    def process_url(self, url: str) -> Dict[str, Any]:
        ...
        if processed_content and processed_content.get('content'):
            ...
            for pdf_info in downloaded_media.get('pdfs', []):
                try:
                    pdf_content_preview = pdf_info.get('content', '')[:500] + "..." if len(pdf_info.get('content', '')) > 500 else pdf_info.get('content', '')
            ...
            print(f"Comprehensive analysis completed with {len(media_summaries['images'])} images and {len(media_summaries['pdfs'])} PDFs")
            ...
            error_msg = processed_content.get('error', 'Unknown error during processing')
            return {
                'content': '',
                'markdown': '',
                'html': '',
