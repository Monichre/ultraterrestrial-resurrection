...
-- STANDARDIZED: All embeddings use 384 dimensions (sentence-transformers all-MiniLM-L6-v2)
-- Date: July 9, 2025
...
-- =====================================================
-- CORE ENTITY TABLES (standardized to 384 dimensions)
-- =====================================================
...
-- =====================================================
-- DOCUMENT PROCESSING TABLES (standardized to 384 dimensions)
-- =====================================================
...
CREATE TABLE document_chunks (
    ...
    page_number INTEGER,
    heading TEXT,
    -- Vector embedding for chunk-level search (STANDARDIZED: 384 dimensions)
    embedding vector(384),
    created_at TIMESTAMP DEFAULT NOW()
);
...
-- Summary files (STANDARDIZED: 384 dimensions)
CREATE TABLE summary_files (
    ...
    metadata JSONB DEFAULT '{}',
    images TEXT,
    -- Vector embedding for summary content (STANDARDIZED: 384 dimensions)
    embedding vector(384),
    created_at TIMESTAMP DEFAULT NOW()
);
...
-- Mindmaps (STANDARDIZED: 384 dimensions)
CREATE TABLE mindmaps (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID REFERENCES users(id),
    json_data JSONB DEFAULT '{}',
    file_url TEXT,
    -- Vector embedding for mindmap content (STANDARDIZED: 384 dimensions)
    embedding vector(384),
    created_at TIMESTAMP DEFAULT NOW(),
    updated_at TIMESTAMP DEFAULT NOW()
);
...
-- =====================================================
-- DOCUMENT EMBEDDINGS TABLE (for main_unified.py)
-- =====================================================

-- Document embeddings for unified ingestion system
CREATE TABLE document_embeddings (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    doc_id TEXT NOT NULL,
    content TEXT NOT NULL,
    -- Vector embedding (STANDARDIZED: 384 dimensions)
    embedding vector(384),
    metadata JSONB DEFAULT '{}',
    source_type TEXT,
    source_url TEXT,
...
-- =====================================================
-- INDEXES FOR PERFORMANCE (all 384 dimensions)
-- =====================================================
...
CREATE INDEX idx_document_embeddings_content_fts ON document_embeddings USING GIN(to_tsvector('english', content));
...
-- =====================================================
-- ADVANCED SEARCH FUNCTIONS (384 dimensions)
-- =====================================================
...
-- Find similar entities across all tables (384 dimensions)
...
COMMENT ON TABLE document_embeddings IS 'Unified document embeddings for main_unified.py ingestion system';

COMMENT ON COLUMN personnel.embedding IS 'Vector embedding (384-dim) for semantic similarity search of personnel bio and role';
COMMENT ON COLUMN documents.embedding IS 'Vector embedding (384-dim) for semantic search of document content';
COMMENT ON COLUMN events.embedding IS 'Vector embedding (384-dim) for semantic search of event descriptions';
COMMENT ON COLUMN document_embeddings.embedding IS 'Vector embedding (384-dim) generated by sentence-transformers all-MiniLM-L6-v2';
