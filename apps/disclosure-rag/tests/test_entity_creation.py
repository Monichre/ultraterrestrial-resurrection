...
async def test_entity_creation():
    ...
    results_file = "/Users/liamellis/Desktop/ultraterrestrial-resurrection/packages/knowledge-base/sources/transcripts/2025-07-12/PmclCzGFotM/entity_processing_results.json"
    ...
    try:
        ...
        for entity_type, results in search_results.items():
            ...
            not_found = len(
                [r for r in results if r.get('status') == 'not_found'])
            total = len(results)

            if total > 0:
                print(
                    f"  {entity_type}: {total} total, ✅ {found} found, ❌ {not_found} need creation")
        ...
        if creation_results.get("error"):
            ...
            print(
                f"✅ Successfully created: {stats.get('total_created', 0)} entities")
            print(
                f"❌ Failed to create: {stats.get('total_failed', 0)} entities")
            print(
                f"📊 Total processed: {stats.get('total_processed', 0)} entities")
