...
class EntityProcessorApp(App):
    ...
    def __init__(self, summary_file: str, video_id: str):
        ...
        self.research_manager = ResearchManager(
            self.research_queue) if RESEARCH_QUEUE_AVAILABLE and self.research_queue else None
    ...
    def on_mount(self) -> None:
        ...
        if XATA_AVAILABLE:
            ...
            self.log_message(
                "⚠️ Xata search not available - check credentials")
    ...
    def setup_tables(self) -> None:
        ...
        results_table.add_columns(
            "Entity", "Status", "Xata Match", "Match Details")

        # Creation results table
        creation_table = self.query_one("#creation-table", DataTable)
        creation_table.add_columns(
            "Entity Name", "Type", "Status", "Xata ID", "Created At")

        # Research queue table
        queue_table = self.query_one("#queue-table", DataTable)
        queue_table.add_columns("Entity Name", "Type",
                                "Priority", "Status", "Created", "Notes")
    ...
    async def extract_entities_worker(self) -> None:
        ...
        try:
            ...
            self.log_message(
                f"📖 Read {len(summary_content)} characters from summary")
            progress.update(progress=25)

            # Extract entities
            self.log_message("🔍 Extracting entities with AI...")
            self.extracted_entities = self.entity_extractor.extract_entities(
                summary_content)
            ...
            total_entities = sum(len(entities)
                                 for entities in self.extracted_entities.values())
    ...
    async def search_entities_worker(self) -> None:
        ...
        if not self.extracted_entities:
            self.log_message(
                "❌ No entities to search - extract entities first")
            return
        ...
        try:
            ...
            for entity_type, entities in self.extracted_entities.items():
                ...
                self.log_message(
                    f"🔍 Searching {len(entities)} {entity_type}...")
                ...
                for entity_name in entities:
                    ...
                    if match:
                        ...
                        self.log_message(
                            f"  ✅ {entity_name} -> {match.get('name', 'Unknown')}")
                    else:
                        self.search_results[entity_type].append({
                            'entity_name': entity_name,
                            'status': 'not_found',
            ...
            self.log_message(
                f"✅ Search complete: {total_matches} matches found")
        ...
        for entity_type, results in self.search_results.items():
            for result in results:
                ...
                results_table.add_row(
                    entity_name, status, match_name, match_details)
    ...
    async def create_entities_worker(self) -> None:
        ...
        if not self.search_results:
            self.log_message(
                "❌ No search results to process - search entities first")
            return
        ...
        try:
            ...
            self.log_message(
                f"🔍 Found {entities_to_create} entities marked for creation")
            progress.update(progress=25)
    ...
    def view_creation_results(self) -> None:
        ...
        if not self.creation_results:
            self.log_message(
                "ℹ️ No creation results available - create entities first")
            return
    ...
    async def add_to_research_queue_worker(self) -> None:
        ...
        try:
            ...
            self.log_message(
                f"📋 Found {entities_to_queue} entities to add to research queue")
            progress.update(progress=25)
                ...
                for result in results:
                    if result.get('status') == 'not_found':
                        ...
                        if task:
                            added_count += 1
                            self.log_message(
                                f"  ✅ Added {entity_name} ({entity_type}) to research queue")
            ...
            self.update_status(
                f"✅ Added {added_count} entities to research queue")
    ...
    def update_queue_table(self) -> None:
        ...
        for task in self.research_queue.tasks.values():
            ...
            notes = task.research_notes[:50] + "..." if len(
                task.research_notes) > 50 else task.research_notes
