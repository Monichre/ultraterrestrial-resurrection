...
class ResearchManager:
    ...
    def __init__(self, queue_manager: ResearchQueueManager):
        ...
        if self.ai_available:
            ...
            logger.warning("⚠️ Research Manager AI not available - missing OpenAI API key")
    ...
    async def generate_research_briefing(self) -> str:
        ...
        try:
            stats = self.queue_manager.get_queue_stats()
            critical_tasks = self.queue_manager.get_tasks_by_priority(ResearchPriority.CRITICAL)
            high_tasks = self.queue_manager.get_tasks_by_priority(ResearchPriority.HIGH)
            queued_tasks = self.queue_manager.get_tasks_by_status(ResearchStatus.QUEUED)
...
"""
    ...
    async def optimize_research_schedule(self) -> Dict[str, Any]:
        ...
        try:
            ...
            unscheduled = self.queue_manager.get_tasks_by_status(ResearchStatus.QUEUED)
            scheduled = self.queue_manager.get_tasks_by_status(ResearchStatus.SCHEDULED)
