...
def run_interactive_loop(agent: Agent):
    ...
    while True:
        choices = [f"{i + 1}. {topic}" for i,
                   topic in enumerate(example_topics)]
        ...
        if answer["topic"] == "Enter custom question...":
            questions = [inquirer.Text(
                "custom", message="Enter your question:")]
...
def deep_knowledge_agent():
    ...
    if session_id is None:
        ...
        if session_id is not None:
            print(
                f"[bold green]Started New Session: {session_id}[/bold green]\n")
        else:
            print("[bold green]Started New Session[/bold green]\n")
    else:
        print(
            f"[bold blue]Continuing Previous Session: {session_id}[/bold blue]\n")
