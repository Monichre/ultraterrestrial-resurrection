...
export default function registerReviewCommands(program: Command): void {
	...
	program
		...
		.action(async (options) => {
			try {
				...
				// Show spinner
				const spinner = ora(
					`Listing files in review bucket for ${dataType}...`,
				).start();

				// Get files
				const files = await fileManager.listFiles(dataType, "review", pattern);
				spinner.succeed(
					`Found ${files.length} files in review bucket for ${dataType}`,
				);

				if (files.length === 0) {
					console.log(
						chalk.yellow(
							`No files found in review bucket for ${dataType} matching pattern "${pattern}"`,
						),
					);
					return;
				}
	...
	program
		...
		.option(
			"-l, --lines <number>",
			"Number of lines to preview (default: 20)",
			"20",
		)
		.action(async (options) => {
			try {
				...
				const fileName =
					options.file || (await promptForSingleFile(dataType, "review"));
				if (!fileName) return;
				...
				try {
					...
					console.log(
						chalk.blue("  Quality Score:    ") +
							formatQualityScore(metadata.qualityScore),
					);
					console.log(
						chalk.blue("  Records:          ") + metadata.recordCount,
					);
					console.log(chalk.blue("  Status:           ") + metadata.status);
					console.log(
						chalk.blue("  Target Table:     ") + metadata.targetTable,
					);
					console.log(
						chalk.blue("  Processing Date:  ") +
					...
					if (totalLines > previewLines) {
						console.log(
							chalk.gray(`... ${totalLines - previewLines} more lines ...`),
						);
					}
					...
					spinner.fail(
						`Failed to preview ${fileName}: ${(error as Error).message}`,
					);
				}
			} catch (error) {
	...
	program
		...
		.option(
			"-a, --all",
			"Approve all files in the review bucket for the specified type",
		)
		.action(async (options) => {
			try {
				// If no type is provided, prompt the user
				...
				console.log(
					chalk.blue(
						`Approving ${filesToApprove.length} file(s) for ${dataType}...`,
					),
				);
	...
	program
		...
		.action(async (options) => {
			try {
				...
				console.log(
					chalk.blue(
						`Rejecting ${filesToReject.length} file(s) for ${dataType}...`,
					),
				);
 ...
 */
async function promptForSingleFile(
