...
export function NodeDropdown({isOpen, onClose}: NodeDropdownProps) {
  ...
  const filteredNodes = nodes.filter((node) =>
    (node.data?.label || node.id).toLowerCase().includes(searchQuery.toLowerCase())
  )

  const isNodeSelected = (id: string) => Boolean(nodes.find((n) => n.id === id && n.selected))
  const selectNode = (id: string) =>
    mindmap?.setNodes(nodes.map((n) => (n.id === id ? {...n, selected: true} : n)))
  const deselectNode = (id: string) =>
    mindmap?.setNodes(nodes.map((n) => (n.id === id ? {...n, selected: false} : n)))
                ...
                <motion.div
                  ...
                  <div
                    className={cn(
                      ...
                      nodeTypeColors[(node.data?.type || 'topic') as NodeType]
                    )}>
                    <span className='text-xs'>
                      {nodeTypeIcons[(node.data?.type || 'topic') as NodeType]}
                    </span>
                  </div>
