                      ...
                      >
                        <div className="flex items-start justify-between">
                          ...
                          <div className="text-blue-400 ml-2">
                            ...
                            {suggestion.type === 'spatial-connection' && <TrendingUp className="w-4 h-4" />}
                            {suggestion.type === 'temporal-context' && <Lightbulb className="w-4 h-4" />}
                          </div>
                        </div>
                      </motion.div>
                    ))}
                ...
                <div className="bg-gray-800/50 border border-gray-600/30 rounded-lg p-3 space-y-2">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {contextualIntelligence.dominantThemes.slice(0, 3).map(theme => (
                      <span key={theme} className="text-xs bg-green-500/20 text-green-300 px-2 py-1 rounded">
                        {theme}
                      </span>
                    ))}
                  </div>
                  <div className="text-xs text-gray-400 mt-2">
                    Era: {contextualIntelligence.temporalFocus.era} ({contextualIntelligence.temporalFocus.yearRange[0]}-{contextualIntelligence.temporalFocus.yearRange[1]})
                  </div>
                  <div className="text-xs text-gray-400">
                    Network Strength: {Math.round(contextualIntelligence.entityNetworkStrength * 100)}%
                  </div>
                </div>
              </div>
            )}
              ...
              <div className="mb-6">
                ...
                <div className="space-y-2">
                  {patternInsights.slice(0, 2).map(insight => (
                    <div key={insight.id} className="bg-purple-900/20 border border-purple-400/30 rounded p-3">
                      <div className="flex justify-between items-center">
                        <span className="text-purple-400 text-sm font-semibold">{insight.type}</span>
                        <span className="text-purple-400 text-xs">{Math.round(insight.confidence * 100)}%</span>
                      </div>
