        ...
        {[
          {
            ...
            label: 'Suggestions',
            icon: Target,
            count: researchSuggestions.length,
          },
          {
            id: 'insights',
            label: 'Insights',
            icon: Lightbulb,
            count: [...spatialInsights, ...patternInsights].length,
          },
          {
            id: 'context',
            label: 'Context',
            icon: TrendingUp,
            count: contextualIntelligence.dominantThemes.length,
          },
        ].map((tab) => (
          <button
            key={tab.id}
        ...
        <AnimatePresence mode='wait'>
              ...
              {researchSuggestions.length === 0 ? (
                <div className='text-center py-8 text-slate-400'>
                  ...
                  <p className='text-xs mt-1'>
                    Add entities to your research canvas to get AI-powered suggestions
                  </p>
                </div>
              ) : (
                  ...
                  <motion.div
                    ...
                    <div className='flex items-start space-x-3'>
                      <div className='text-blue-400 mt-0.5'>
                        ...
                        {suggestion.type === 'narrative-bridge' && (
            ...
            <motion.div
              ...
              {[...spatialInsights, ...patternInsights].length === 0 ? (
                <div className='text-center py-8 text-slate-400'>
                  ...
                  <p className='text-xs mt-1'>
                    Pin more entities to discover patterns and connections
                  </p>
                </div>
              ) : (
                [...spatialInsights, ...patternInsights].map((insight, index) => (
                  <motion.div
                    ...
                    <div className='flex items-start justify-between mb-2'>
                      <h4 className='text-purple-300 text-sm font-medium'>{insight.title}</h4>
                      <span className='text-purple-400 text-xs'>
                        {Math.round(insight.confidence * 100)}%
                      </span>
                    </div>
                    <p className='text-slate-300 text-xs leading-relaxed mb-2'>{insight.summary}</p>
                    <div className='flex items-center justify-between'>
          ...
          {activeTab === 'context' && (
            <motion.div
              ...
              <div>
                ...
                <div className='bg-slate-800/50 border border-slate-600/30 rounded p-3'>
                  ...
                  {contextualIntelligence.spatialRelevance.locations.length > 0 && (
                    <div className='text-slate-400 text-xs mt-1'>
                      Key locations:{' '}
                      {contextualIntelligence.spatialRelevance.locations.slice(0, 3).join(', ')}
                    </div>
                  )}
                </div>
              </div>
              ...
              {/* Smart Search Rules */}
              <div>
                <h4 className='text-slate-300 text-sm font-medium mb-2'>Current Search Context</h4>
                <div className='bg-slate-800/50 border border-slate-600/30 rounded p-3'>
