...
export function PinnedCardsCanvas({ 
  ...
  const handleConnectionComplete = useCallback((targetCard: PinnedCard) => {
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <p className="text-slate-400 max-w-md text-sm">
                Pin cards from your exploration to begin deep analysis. Connect entities, 
                analyze relationships, and uncover patterns.
              </p>
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
      ...
      <AnimatePresence>
        {pinnedCards.map((card) => (
          <PinnedResearchCard
            ...
            isConnectionTarget={connectionMode.isActive && connectionMode.sourceCard?.id !== card.id}
  ...
  return (
    <div className="bg-white rounded-sm shadow-xl border-8 border-white w-48 overflow-hidden">
      ...
      <div className="p-3 space-y-1">
        ...
        {card.data?.role && (
          <p className="text-gray-600 text-xs truncate">{card.data.role}</p>
        )}
...
function StandardCard({ card }: { card: PinnedCard }) {
  const getEntityIcon = (type: string) => {
    switch (type) {
      ...
      case 'organizations': return <MapPin className="w-4 h-4" />
      case 'documents': return <FileText className="w-4 h-4" />
      default: return <span className="w-4 h-4 rounded bg-current" />
    }
  }
