...
const PAGE_VARIANTS: PageMotion[] = [
  {
...
export type FolderInteractionProps = {
  /** Controlled open state. When omitted, component manages its own toggle. */
  open?: boolean
  defaultOpen?: boolean
...
export function FolderInteraction({
  ...
  const clipPathId = `folder-bgblur-${reactId}`
  const fillGradId = `folder-fill-${reactId}`
  const strokeGradId = `folder-stroke-${reactId}`
  ...
  return (
    <div className={`flex w-full items-center justify-center ${className}`}>
      <div
        ...
        className='wrapper relative h-52 w-80 cursor-pointer'
      ...
      >
        ...
        <motion.div
          ...
          className='absolute -bottom-[1px] -left-[1px] -right-[1px] z-20 flex h-44 origin-bottom items-center justify-center overflow-visible rounded-3xl'
        ...
        >
          ...
          >
            ...
            />
            <defs>
              <clipPath id={clipPathId} transform='translate(13 13)'>
                <path d={folderPath} />
              </clipPath>
              <linearGradient
                id={fillGradId}
