...
describe( 'graph compiler', () => {
  ...
  it( 'reveals the opening spine during overview so the canvas is not blank', () => {
    const state = {
      ...createInitialRuntimeState( nuclearShadowDefinition ),
      hydrated: true,
    ...
    expect( graph.nodes[0].data.status ).toBe( 'active' )
    expect( graph.nodes[1].data.status ).toBe( 'ghost' )
    expect( graph.nodes[2].data.status ).toBe( 'ghost' )
    expect( graph.nodes[0].hidden ).toBe( false )
  } )
} )
