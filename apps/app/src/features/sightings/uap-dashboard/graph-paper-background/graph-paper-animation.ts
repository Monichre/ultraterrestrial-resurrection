const tools = {
  ...
  random(min: number, max: number, int?: boolean) {
    ...
    return int ? Number.parseInt(result.toString()) : result
  },
...
const context = {
  plane: null as CanvasRenderingContext2D | null,
...
export class GraphPaperAnimation {
  ...
  constructor(onInitialized?: () => void) {
    this.onInitialized = onInitialized || null
    this.state = {
      area: 0,
      time: Date.now(),
      lt: 0,
  ...
  canvasInit() {
    ...
    // Try to get contexts from the window object if they were set there
    if (typeof window !== "undefined" && window.graphPaperContext) {
      context.plane = window.graphPaperContext.plane
      context.main = window.graphPaperContext.main
    }

    // If still not available, try to get them directly
    if (!context.plane) {
      const planeCanvas = document.getElementById("plane-canvas") as HTMLCanvasElement
      if (planeCanvas) {
        context.plane = planeCanvas.getContext("2d")
      }
    }
    ...
    if (!context.main) {
      ...
      if (mainCanvas) {
        context.main = mainCanvas.getContext("2d")
      }
    }

    if (!context.plane || !context.main) {
      console.error("Failed to get canvas contexts")
      return
    }

    // Rest of the method remains the same
    context.plane.lineCap = lineCapAndJoin as CanvasLineCap
    context.plane.lineJoin = lineCapAndJoin as CanvasLineJoin
    context.plane.font = font
    context.plane.fillStyle = color
    context.plane.strokeStyle = color

    context.main.lineCap = lineCapAndJoin as CanvasLineCap
    context.main.lineJoin = lineCapAndJoin as CanvasLineJoin
  ...
  resizeHandler(e?: Event) {
    ...
    if (planeCanvas && mainCanvas) {
      planeCanvas.height = doc.height
      planeCanvas.width = doc.width
      mainCanvas.height = doc.height
      mainCanvas.width = doc.width
    }
  ...
  loop() {
    const loop = () => {
      ...
      // Check if we should signal initialization is complete
      if (state.planeProgress > 0.3 && !this.hasSignaledReady && this.onInitialized) {
  ...
  drawNumbersAnimation() {
    ...
    state.animNumbers.forEach((item, i) => {
      ...
      tools.drawPath(ctx, () => {
        ...
        const th = Number.parseInt(ctx.font) || 18
  ...
  startGeneratingLines() {
    ...
    const generateItem = () => {
      ...
      const item = {
        ...
        x: tools.random(0, cells[0], true) * xCell,
        y: tools.random(0, cells[1], true) * yCell,
        coord: tools.random(0, 1, true) ? "x" : "y",
        length: 0,
        dir: 0,
        distance: 0,
      }
  ...
  drawMouseMoveInteraction(props: { i: number; i2: number; x: number; y: number }) {
    const ctx = context.main
    const state = this.state
  ...
  drawPlaneDotsAnimation(props: { dp: number; i: number; i2: number; x: number; y: number }) {
    ...
    if (!mf) return
    tools.drawPath(ctx, () => {
      ctx.fillStyle = `rgba(255,255,255,${mf * 0.3})` // Increased from 0.15 to 0.3
      ctx.fillRect(x - 1, y - 1, size, size)
    })
  }

  drawPlaneCenterLines(props: { p: number }) {
    const { p } = props
    const ctx = context.plane
    ...
    tools.drawPath(ctx, () => {
      ctx.fillStyle = `rgba(255,255,255,${0.4 + (1 - p) * 1})` // Increased from 0.2 to 0.4
      ctx.fillRect(centerCoords[0], 0 + (doc.height / 2) * (1 - p), 1, doc.height * p)
      ctx.fillRect(0 + (doc.width / 2) * (1 - p), centerCoords[1], doc.width * p, 1)
    })
  }

  drawYLines(props: { i: number; i2: number; p: number; cp: number; x: number; y: number }) {
    ...
    if (i) {
      tools.drawPath(ctx, () => {
        ctx.fillStyle = `rgba(255,255,255,${0.1 + (1 - p) * 0.5})` // Increased from 0.05 to 0.1 and 0.35 to 0.5
        ctx.fillRect(x, 0 + (doc.height / 2) * (1 - ef), 1, doc.height * ef)
      })
    }
  }

  drawYMarkup(props: { i: number; p: number; cp: number; x: number; y: number }) {
    const ctx = context.plane
  ...
  drawXLines(props: { i: number; i2: number; p: number; cp: number; x: number; y: number }) {
    const ctx = context.plane
  ...
  drawXMarkup(props: { i2: number; p: number; cp: number; x: number; y: number }) {
    const ctx = context.plane
  ...
  drawMarkupYAnimation(props: { i: number; i2: number; x: number; y: number; cp: number }) {
    const ctx = context.main
  ...
  drawMarkupXAnimation(props: { i2: number; x: number; y: number; cp: number }) {
    const ctx = context.main
