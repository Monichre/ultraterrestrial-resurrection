...
export const SightingsGlobe = ({ geoJSONSightings }: SightingsGlobeProps) => {
	...
	useEffect(() => {
		...
		try {
			...
			if (
				...
				mapInstance.getSource("sightings-source")
			) {
				...
				if (mapInstance.getLayer("clusters"))
					mapInstance.setFilter("clusters", timeFilter);
		...
		try {
			...
			if (useTileset) {
				...
				if (!map || typeof map.getSource !== "function") {
					console.error(
						"Invalid map object or map.getSource is not a function in cleanup",
						map,
					);
					return;
				}
				...
				if (map.getSource("sightings-tileset")) {
					if (map.getLayer("sightings-points"))
						map.removeLayer("sightings-points");
			...
			mapInstance.on("click", "clusters", (e) => {
				...
				if (pointCount < 20) {
					...
					if (
						...
						console.error(
							"Invalid sightings source or getClusterExpansionZoom is not a function",
						);
						return;
					}
		...
		const handleMapLoad = () => {
			...
			if (instance && typeof instance.getSource === "function") {
				...
				if (retryCount < MAX_RETRIES) {
					...
					console.error(
						"Max retries reached while waiting for map methods to become available",
					);
	...
	const buildVisualizationLayers = useCallback(() => {
		...
		if (
			!Array.isArray(filteredSightings.features) ||
		...
		if (visualizationMode === "arcs" || visualizationMode === "both") {
			try {
				...
				const features = validFeatures.slice(
					0,
					Math.min(validFeatures.length, 100),
				);

				for (let i = 0; i < features.length; i++) {
					const feature1 = features[i] as GeoJSONFeature;
					const date1 = new Date(
						feature1.properties?.date || feature1.properties?.timestamp || 0,
					);

					for (let j = i + 1; j < features.length; j++) {
						const feature2 = features[j] as GeoJSONFeature;
						const date2 = new Date(
							feature2.properties?.date || feature2.properties?.timestamp || 0,
						);
						...
						if (timeDiff < 7 * 24 * 60 * 60 * 1000) {
							connections.push({
								...
								sourceName:
									feature1.properties?.city || feature1.properties?.location,
								targetName:
									feature2.properties?.city || feature2.properties?.location,
		...
		return (
			<div className="h-screen w-screen relative flex items-center justify-center bg-gray-900">
				<div className="text-white font-monumentMono text-xl p-4 bg-black bg-opacity-70 rounded-lg">
					<div className="mb-2">
						Loading sightings data from {SIGHTINGS_GEOJSON_PATH}...
					</div>
				...
				{popupInfo && (
					...
					>
						<div className="p-2 max-w-md bg-black bg-opacity-80 text-white rounded-md border border-cyan-500/30 relative">
							...
							popupInfo.properties.id ===
								currentHighlightedEvent.properties.id ? (
							...
							{(popupInfo.properties.video || popupInfo.properties.image) && (
								<div className="mt-2 flex gap-2">
									{popupInfo.properties.image && (
										<span className="text-xs bg-indigo-800 px-2 py-0.5 rounded">
											📷 Photo
										</span>
									)}
			...
			<div className="absolute bottom-4 left-4 p-4 bg-black bg-opacity-70 text-white rounded-lg z-10 font-monumentMono">
				...
				<div className="mb-3">
					...
					<input
						...
						value={timeRange[0]}
						onChange={(e) =>
							setTimeRange([Number.parseInt(e.target.value), timeRange[1]])
						}
					...
					<input
						...
						value={timeRange[1]}
						onChange={(e) =>
							setTimeRange([timeRange[0], Number.parseInt(e.target.value)])
						}
						className="w-full"
					/>
				</div>
			...
			<div className="absolute top-4 right-4 p-4 bg-black bg-opacity-70 text-white rounded-lg z-10 font-monumentMono">
				...
				{useExtData && (
					<p className="text-sm text-cyan-300 mt-2">
						Using {sightings?.features?.length || 0} sightings from external
						data
					</p>
				)}
				...
				{isAnimating && (
					<div className="mt-2 border-t border-cyan-800 pt-2">
						<p className="text-sm text-yellow-400 font-bold">
							Time Animation Active
						</p>
						<p className="text-xs">
							Currently showing:{" "}
							{format(new Date(animationYear, animationMonth), "MMMM yyyy")}
						</p>
						...
						{filteredSightings?.features && (
							<div className="mt-1 text-xs">
								<span className="text-yellow-300">
									{filteredSightings.features.filter((f) =>
										isSignificantEvent(f),
									)?.length || 0}{" "}
									significant events
								</span>
								{" / "}
								<span>
									{filteredSightings.features?.length || 0} total sightings
								</span>
							</div>
						)}
					</div>
