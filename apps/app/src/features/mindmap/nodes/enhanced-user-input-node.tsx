...
const EnhancedUserInputNode = memo<EnhancedUserInputNodeProps>(function EnhancedUserInputNode(props) {
  ...
  const getContextualInsights = useCallback(() => {
    ...
    return {
      ...
      timeframe: graphContext.timelineBounds.earliest && graphContext.timelineBounds.latest 
            ...
            {isContextual && contextualInsights && (
              ...
              >
                ...
                {contextualInsights.keyFigures.length > 0 && (
                  <div>
                    <div className="text-xs text-white/60 uppercase tracking-wide mb-1">Key Figures</div>
          ...
          {(data.answer || data.streamingAnswer) && (
            <div className="space-y-3">
              ...
              <div className="text-sm text-white/90 leading-relaxed">
                {showFullAnswer 
                  ? (data.answer || data.streamingAnswer)
                  : `${(data.answer || data.streamingAnswer || '').slice(0, 200)}${(data.answer || data.streamingAnswer || '').length > 200 ? '...' : ''}`
                }
              </div>
              
              {(data.answer || data.streamingAnswer || '').length > 200 && (
                <button
