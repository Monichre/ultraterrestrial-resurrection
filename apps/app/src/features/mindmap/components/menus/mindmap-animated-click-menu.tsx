...
export function ExplodingMenuItem({item, index}: {item: any; index: number}) {
...
export function MindMapAnimatedClickMenu({isOpen, closeMenu, clickPosition}: any) {
  ...
  const actions = [
    ...
    },
    {
      render: () => (
        <motion.button
          onClick={() => handleLoadingRecords({data: {type: 'organizations'}})}
          className='flex h-8 w-8 cursor-move items-center justify-center rounded-full bg-mauve-light-3'>
          ...
          <motion.div
            ...
            <ul className='relative '>
              ...
              {actions.map((item, index) => {
                return <ExplodingMenuItem item={item} index={index} key={index} />
              })}
            </ul>
          </motion.div>
        )}
