...
export const ThreadBoard = () => {
  ...
  const createNewThread = () => {
    const newThread: Thread = {
      ...
      description: 'Initialized thread. Awaiting data connections and analysis framework.',
    }
  ...
  const handleFileDrop = (threadId: string) => {
    ...
    setThreads(
      threads.map((thread) => (thread.id === threadId ? {...thread, sourceVerified: true} : thread))
    )
  }
    ...
    <motion.div
      ...
      {!isExpanded && (
        <motion.div
          ...
          // top-1/2
          // rotate-[-2deg]
          className='absolute right-0 z-50 top-[10px] w-12 h-24 bg-black/40 backdrop-blur-sm border-l border-y border-[#adf0dd]/30 rounded-l-md flex items-center justify-center cursor-pointer  shadow-lg'
