...
const VIEWS: ViewItem[] = [
  {
    ...
    description: 'Assemble records into an investigation with clearly delineated AI inference',
...
export function FullScreenMenu({isOpen: isOpenProp, onClose: onCloseProp}: FullScreenMenuProps) {
  ...
  const views: ViewItem[] = VIEWS.map((view) =>
    view.id === 'canvas'
      ? {
          ...view,
          stats: [
            {label: 'Records', value: String(nodes.length)},
  ...
  const onClose = onCloseProp ?? (() => setFullScreenMenuOpen(false))
