...
import { getIncidentById } from "@/data/ufo-sightings"
import { useLocalIncidents } from "@/lib/use-local-incidents"

function IncidentDetailContent() {
  const searchParams = useSearchParams()
  const id = searchParams.get("id")
  const { incidents, isLoading } = useLocalIncidents()
  const incident = useMemo(() => {
    if (!id) return null
    return incidents.find((candidate) => candidate.id === id) || getIncidentById(id) || null
  }, [id, incidents])
      ...
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-muted-foreground mb-6">
            {isLoading
              ? "Loading the local CSV backend..."
              : "The incident you are looking for does not exist in our database."}
          </p>
          <Link
            href="/ufo-sightings"
  ...
  const relatedIncidents = incident.relatedIncidents
    .map((relId) => incidents.find((candidate) => candidate.id === relId) || getIncidentById(relId))
