...
import { useLocalIncidents } from "@/lib/use-local-incidents"
...
export default function UFOSightingsPage() {
  const { incidents, stats, isLoading, error } = useLocalIncidents()
  ...
  const filteredIncidents = useMemo(() => {
    ...
    return filtered
  }, [incidents, searchQuery, selectedClassification, selectedCredibility, selectedDecade, sortBy])
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
            <p className="text-sm text-muted-foreground">
              Showing {filteredIncidents.length} of {incidents.length} local records
            </p>
            <div className="flex flex-wrap items-center gap-2">
              {isLoading && <Badge variant="outline">Loading local CSVs...</Badge>}
              {stats && (
                <Badge variant="outline">
                  CSV backend: {stats.government} gov / {stats.events} events / {stats.sightings} sightings
                </Badge>
              )}
              {error && <Badge variant="destructive">Static fallback</Badge>}
            </div>
          </div>
        </div>
      </section>
