...
export default function NetworkTimelineExplorer() {
  const { incidents, stats } = useLocalIncidents()
  ...
  {/* Main Canvas Area */}
      ...
      >
        ...
        <div className="absolute left-6 bottom-8 flex gap-3">
          ...
          {stats && (
            <div className="bg-card/90 backdrop-blur-sm border border-border rounded-xl px-4 py-2">
              <span className="text-2xl font-bold text-chart-4">{stats.total.toLocaleString()}</span>
              <span className="text-sm text-muted-foreground ml-2">Local CSV rows</span>
            </div>
          )}
        </div>
      </div>
