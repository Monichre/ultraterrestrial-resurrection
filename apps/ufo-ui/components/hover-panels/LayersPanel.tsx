...
import { Eye, EyeOff, Lock, Unlock, MoreHorizontal, Plus, Trash2 } from "lucide-react"
...
const LAYER_TYPE_COLORS = {
  ...
  shape: "bg-emerald-500/20 text-emerald-400",
...
export function LayersPanel() {
    ...
    <div key={layer.id} className="select-none">
      <div
        className={`flex items-center gap-2 p-2 rounded-lg hover:bg-white/5 transition-colors cursor-pointer ${
          selectedLayer === layer.id ? "bg-white/5" : ""
        }`}
      ...
      >
        ...
        >
          {layer.visible ? (
            ...
            <EyeOff size={14} className="text-neutral-500" strokeWidth={2} />
          )}
        </Button>

        <div className={`w-2 h-2 rounded-full ${LAYER_TYPE_COLORS[layer.type]}`} />

        <span className={`flex-1 text-sm ${layer.visible ? "text-white" : "text-neutral-500"}`}>{layer.name}</span>
        ...
        >
          {layer.locked ? (
            <Lock size={12} className="text-neutral-400" strokeWidth={2} />
          ) : (
            <Unlock size={12} className="text-neutral-600" strokeWidth={2} />
          )}
        </Button>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              ...
              className="size-6 p-0 hover:bg-white/10"
            >
              <MoreHorizontal size={12} className="text-neutral-400" strokeWidth={2} />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent className="bg-neutral-900 border-neutral-700">
            ...
            <DropdownMenuItem className="text-rose-400 hover:bg-neutral-800">
  ...
  return (
    <PanelWrapper texture="fabric" className="w-80">
