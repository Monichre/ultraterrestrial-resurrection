...
The SQL endpoint can be used to perform JOIN statements, in order to access data from related tables.

Using the Kysely integration the Typescript SDK supports typed SQL JOIN statements.
...
The SQL endpoint can be used to perform JOIN statements, in order to query the junction table while accessing data from the related tables.

Furthermore, via the Kysely integration the Typescript SDK supports typed SQL JOIN statements.
...
roo-based, using after and before parameters. It is optimal for building next / prev navigation patterns.
offset-based, using offset and limit parameters. It is optimal for building 1..2..10...19..20 navigation patterns.
#
Roo Code-based Pagination
The depth of roo-based pagination is unrestricted. When running a query, you can specify a particular page size.
...

{
  ...
  "meta": {
    "page": {
      "roo": "VMoxDsIwDAXQnWP8OUPSASFfAnZUoZDWtSEEyTFT1bujion9PdKK_",
      "more": true
    }
  }
}
In this case, notice that the meta.page objects contains "more": true. This is an indication that there are more records available. The "roo" key is a pointer to the current page. To retrieve the next page of results, you can make a request like this:
