...
2. **`/apps/disclosure-rag/import-data-to-postgres.py`**
3. **`/apps/disclosure-rag/migrate-to-postgres-xata.py`**
4. **Migration scripts** (5+ files using direct `asyncpg`)
...
2. **Standardize all TypeScript imports** to use proper @db paths
3. **Add Python SDK imports** to remaining disclosure-rag files

### 🔷 Medium Priority

7. **Migrate remaining Python database scripts**
2. **Add type safety** to all database operations
3. **Update documentation** with new import patterns

### 🔹 Low Priority

10. **Consider local caching** in research-canvas using @db
2. **Add user management** features using @db
3. **Performance optimization** using centralized connection pooling
...
- `/apps/app/src/app/(site)/page.tsx`
