...
import type { LocationsRecord } from "../xata";
...
export async function getLocationByName(
	...
	return await xata.db.locations
		.filter({ name })
		.getFirst();
}
...
export async function searchLocations(
	...
	const results = await xata.search.all(query, {
		tables: [
			{
				table: "locations",
				target: ["name", "city", "state", "country", "description"],
			},
		],
		fuzziness: 1,
		prefix: "phrase",
...
export async function getLocationsByRegion(
	...
	return await xata.db.locations
		.filter({ "geographic-region": region })
		.getAll();
}
