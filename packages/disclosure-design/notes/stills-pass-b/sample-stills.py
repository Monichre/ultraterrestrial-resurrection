#!/usr/bin/env python3
"""Pass B: pixel-sample every png/jpg/webp still. Writes palettes.json only in this namespace."""
...
OUT_DIR = ROOT / "notes" / "stills-pass-b"
...
def main() -> int:
    stills = list_stills()
    json_path = OUT_DIR / "palettes.json"
    errors: list[dict] = []
    rows: list[dict] = []
    ...
    for i, path in enumerate(stills, 1):
        try:
            ...
            rec = {
                ...
                "bucket": pass_b_bucket(path.relative_to(ROOT).as_posix()),
            }
            errors.append(rec)
        rows.append(rec)
        if i % 25 == 0 or i == len(stills):
            print(
                f"{i}/{len(stills)}",
                rec.get("path", ""),
                rec.get("dominant", rec.get("error")),
                flush=True,
            )

    buckets = defaultdict(int)
    for r in rows:
        buckets[r.get("bucket", "?")] += 1
