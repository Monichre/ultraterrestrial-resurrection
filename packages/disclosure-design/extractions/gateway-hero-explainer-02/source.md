---
title: source — gateway-hero-explainer-02
description: Provenance + pixel facts for gateway-hero-explainer-02. Vision not yet per-image; cluster-inherited take marked honestly.
type: note
created: 2026-08-13
author: agent
tags: [folderize, source, batch]
---

# Source — `gateway-hero-explainer-02`

> **Provenance: cluster-inherited.** This still was not individually vision-read.
> Cluster is homogeneous (same prompt / same set) — the take below is inherited from the gold rep [gateway-hero-explainer](../gateway-hero-explainer/) and is very likely accurate for this frame.

## Pixel facts (measured, this exact file)

- 1024×1536 PNG · 2851 KB · mean luma 21.1 (dark register)
- median `#0c0c0c` · dominant `#1c1917` · chroma peak `#1c1917`
- swatches: `#1c1917` (17.1%), `#42453f` (9.9%)

## Cluster take (inherited)

Hero + process explainer (teal→red ladder)

## aesthetics.md thinkingprocess — status per heading

Applied per `prompts/aesthetics.md`; values marked *inherited* come from the cluster rep, *measured* from this file's pixels, *pending* needs a per-image vision pass.

- **Aspects/Subjects/Motifs**: *inherited* — Hero + process explainer (teal→red ladder)
- **Adjectives**: *pending*
- **Display/Medium/Usage-Context**: *inherited from filename/cluster* — Midjourney/design-lab still
- **Genres/Styles**: *pending*
- **Color Palette**: *measured* — see pixel facts above and `design-tokens.md`
- **Composition**: *pending*
- **Emotional Impact**: *pending*
- **Other Details**: original filename `gateway-hero.png`

Upgrade path: vision-read this frame and rewrite in the full shorthand paragraph per `prompts/aesthetics.md` (100–150 words, code block).
