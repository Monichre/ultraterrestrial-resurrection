...
import { DataTypeConfig, BucketConfig } from '../config';
...
export interface FileMetadata {
  ...
  // Status tracking
  status: 'processing' | 'review' | 'approved' | 'rejected' | 'inserted' | 'failed';
...
function getMetadataFilePath(
  ...
  return path.join(fileManager.getBucketPath(dataType, bucketStage), metadataFileName);
}
...
export function updateMetadata(
  ...
  // Add workflow step if status changed
...
/**
 * Move metadata with a file from one bucket to another
 * @param dataType The type of data
 * @param sourceStage The source bucket
 * @param targetStage The target bucket
 * @param fileName The name of the file
 * @param statusUpdate Optional status update to apply
 */
export async function moveMetadata(
  ...
  try {
    ...
    const metadata = await readMetadata(dataType, sourceStage, fileName);
    
    // Update status if provided
    if (statusUpdate) {
      metadata.status = statusUpdate.status;
      metadata.statusMessage = statusUpdate.message;
      
      // Add workflow step
      metadata.workflowSteps.push({
        stage: `moved_to_${targetStage}`,
        timestamp: new Date().toISOString(),
    ...
    await writeMetadata(dataType, targetStage, fileName, metadata);
    
    // Delete metadata from source
    const sourceMetadataPath = getMetadataFilePath(dataType, sourceStage, fileName);
    if (fs.existsSync(sourceMetadataPath)) {
      fs.unlinkSync(sourceMetadataPath);
    }
  } catch (error) {
    throw new MetadataError(
      `Failed to move metadata for file ${fileName} from ${sourceStage} to ${targetStage} for ${dataType}: ${(error as Error).message}`,
      error as Error
    );
  }
}
