...
export const postgresTools = (
  ...
  const tools: Partial<Record<PGTools, Tool>> = {
    ...
    getExplainForQuery: tool({
      description:
        "Analyzes and optimizes a given SQL query, providing detailed execution plan in JSON format. If the query is not valid, it should return an error message. The function itself will add EXPLAIN keyword to so you don't need include it.",
      parameters: z.object({
