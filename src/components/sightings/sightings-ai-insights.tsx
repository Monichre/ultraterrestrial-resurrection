...
import type { 
  ...
  Anomaly,
  Correlation
} from "@/services/sightings/actions/sightings";
...
const GeographicClustersChart = ({ clusters }: { clusters: GeoCluster[] }) => {
  if (!clusters || clusters.length === 0) return <EmptyState message="No geographic cluster data available" />;
...
const TemporalPatternsChart = ({ patterns }: { patterns: TemporalPattern[] }) => {
  if (!patterns || patterns.length === 0) return <EmptyState message="No temporal pattern data available" />;
...
const ShapePatternsChart = ({ shapes }: { shapes: ShapePattern[] }) => {
  if (!shapes || shapes.length === 0) return <EmptyState message="No shape pattern data available" />;
...
const CorrelationsVisualization = ({ correlations }: { correlations: Correlation[] }) => {
  if (!correlations || correlations.length === 0) return <EmptyState message="No correlation data available" />;
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        ...
        <CardDescription>Correlations with military bases and other significant locations</CardDescription>
      </CardHeader>
      <CardContent className="h-[300px]">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart
...
const ErrorState = ({ error }: { error: string }) => (
  <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4 text-center">
    ...
    <p className="text-sm text-red-600 dark:text-red-400 mt-2">{error || 'An error occurred while analyzing sightings data'}</p>
    <button 
