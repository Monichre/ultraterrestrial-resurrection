...
import { PlusIcon, MagnifyingGlassIcon, Cross2Icon, ChevronLeftIcon, ChevronDownIcon } from "@radix-ui/react-icons"
...
export function DynamicToolbar() {
  ...
  return (
    <div className="bg-neutral-950 bg-gradient-to-b from-black/90 border border-gray-200 border-zinc-200 dark:border-zinc-800 rounded-xl p-2 shadow-lg w-fit relative dark:border-gray-800">
      ...
      <div className="flex items-center gap-2 h-10">
        ...
        <div className="relative flex items-center">
          <AnimatePresence mode="wait">
            {isSearchExpanded ? (
              ...
              >
                <MagnifyingGlassIcon className={cn(iconClass, isSearchFocused && "text-[#0C8CE9] dark:text-[#adf0dd]")} />
                <motion.div
                ...
                >
                  ...
                  {isSearchFocused && searchValue.length > 0 && (
                    ...
                    >
                      <Cross2Icon className={iconClass} />
                    </Button>
                  )}
                </motion.div>
              ...
              >
                <MagnifyingGlassIcon
                  className={cn(
                    iconClass,
                    isSearchFocused ? "text-zinc-900 dark:text-[#adf0dd]" : "text-[#0C8CE9] dark:text-[#adf0dd]",
                  )}
                />
              </motion.button>
            )}
          ...
          <DropdownMenuContent align="end" className="w-48 bg-zinc-900 text-zinc-100">
            <AnimatePresence mode="wait">
              {options.map((option) => (
                ...
                >
                  <DropdownMenuItem className="text-zinc-100 bg-zinc-900 hover:text-[#0C8CE9] hover:bg-zinc-800" onClick={() => setSelectedOption(option)}>{option}</DropdownMenuItem>
                </motion.div>
              ))}
            </AnimatePresence>
          </DropdownMenuContent>
        ...
        <Button variant="ghost" size="icon" onClick={() => handleButtonClick("User")} className="h-10 w-10 p-0">
          ...
          >
            ...
            ></path>
            <path
              d="M0.877075 7.49988C0.877075 3.84219 3.84222 0.877045 7.49991 0.877045C11.1576 14.1227 7.49988C14.1227 11.1575 11.1576 14.1227C3.84222 0.877075 7.49988ZM7.49991 1.82704C4.36689 1.82704 1.82708 4.36686 7.49988C1.82708 10.6329 4.36689 13.1727 13.1727C10.6329 7.49988C13.1727 1.82704Z"
              fill="#adf0dd"
              fillRule="evenodd"
              clipRule="evenodd"
              pathLength="1"
