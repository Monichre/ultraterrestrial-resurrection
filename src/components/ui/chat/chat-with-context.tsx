...
import { PaperPlaneIcon, FileText, Globe, Loader2 } from 'lucide-react';
...
export function ChatWithContext({
  ...
  const handleResourceSubmit = async (values: {
    ...
    try {
      ...
      originalSubmitMessage(`There was an error processing your resource: ${err.message}. Please try again.`);
    } finally {
    ...
    <ExpandableChat position={position} size={size} icon={icon}>
        ...
        <TabsContent value="chat" className="mt-0 h-full">
          <ScrollArea className="h-full pr-4">
            ...
            {resources.length > 0 && activeResource && (
              <div className="mb-4 p-3 bg-accent/20 rounded-lg text-sm">
                <p className="font-medium mb-1">Resource: {resources.find(r => r.resourceId === activeResource)?.title}</p>
                <p className="text-muted-foreground">{resources.find(r => r.resourceId === activeResource)?.summary}</p>
              </div>
            )}
          ...
          <ScrollArea className="h-full pr-4">
            <div className="space-y-4">
              <ChatResourceForm 
                ...
                isProcessing={processingResource}
              />
                    ...
                    {resources.map((resource) => (
                      <div 
                        ...
                        className={cn(
                          ...
                          activeResource === resource.resourceId ? "border-primary bg-primary/5" : "border-muted"
                        )}
                        onClick={() => {
                      ...
                      >
                        <div className="flex items-center">
                          ...
                          <span className="text-sm">
                            {resource.title.length > 30 ? `${resource.title.substring(0, 30)}...` : resource.title}
                          </span>
                        </div>
      ...
      <ExpandableChatFooter className="border-t p-4 bg-background">
        <form onSubmit={handleSubmit} className="flex space-x-2">
          ...
          />
          <Button type="submit" size="icon" disabled={isLoading || !input.trim() || activeTab !== 'chat'}>
