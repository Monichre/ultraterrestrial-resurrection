...
export function useSightingsData({
  ...
  const getTimeChunks = useCallback((range: TimeRange): TimeRange[] => {
    const chunks: TimeRange[] = [];
    for (let year = range.startYear; year <= range.endYear; year += chunkSize) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const filteredSightings = sightingsData.sightings.filter(sighting => {
        // Shape filter
        if (filters.shape && sighting.category && 
            !sighting.category.includes(filters.shape.toLowerCase())) {
          return false;
        }
        
        // Location filter (basic implementation)
        if (filters.location && sighting.location.city && 
            !sighting.location.city.toLowerCase().includes(filters.location.toLowerCase())) {
          return false;
        }
