...
const MessageContent = ({children, markdown = false, className, ...props}: MessageContentProps) => {
  const classNames = cn(
    'rounded-lg p-2 text-gray-950 bg-gray-100 prose break-words whitespace-normal dark:text-gray-50 dark:bg-gray-800',
    className
  )
...
const MessageActions = ({children, className, ...props}: MessageActionsProps) => (
  <div
    className={cn('text-gray-500 flex items-center gap-2 dark:text-gray-400', className)}
    {...props}>
    {children}
  </div>
)
