...
const nodeWidth = CHILD_DIMENSIONS.width
const nodeHeight = CHILD_DIMENSIONS.height
...
export const layoutElementsTreeFlex = ( tree: any, rootId: any, direction: 'TB' | 'LR' = 'TB' ) => {
  const isTreeHorizontal: any = direction === 'LR'
  ...
  entitreeEdges.forEach( ( edge: Edge ) => {
  ...
  entitreeNodes.forEach( ( node: Node ) => {
    const newNode: any = {}
