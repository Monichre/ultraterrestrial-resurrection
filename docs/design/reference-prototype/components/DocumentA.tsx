...
const defaultProps: Required<DocumentAProps> = {
  header: {
    title: "UN:DE FECTAL",
  ...
  return (
    <DocumentFrame className="mt-12">
      ...
      <div className="relative px-6 pb-10">
            ...
            <div className="space-y-2 text-[10px] tracking-wider">
              {config.sidebar.sections.map((section, index) => (
                <div key={index} className="border border-black/40 p-2">
                  ...
                  {section.subtitle && <div className="text-[9px] opacity-70">{section.subtitle}</div>}
                  {section.items && (
        ...
        <div className="absolute bottom-2 right-4 text-sm font-bold tracking-widest">{config.footer.codeReference}</div>
      </div>
    </DocumentFrame>
  )
}
