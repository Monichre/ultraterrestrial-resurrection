...
import { FONT_ANTON, FONT_SPECIAL_ELITE, FONT_CAVEAT } from "@/lib/fonts"
...
const DEFAULT_PROPS: Required<WeatheredClassifiedDocumentProps> = {
  ...
  photoCaption: {
    labelTop: "Field Evidence",
    captionNote: "Witness account verified - object exhibited controlled flight patterns",
  },
}

export function WeatheredClassifiedDocument(props: WeatheredClassifiedDocumentProps = {}) {
  const config = {
    showNavigation: props.showNavigation ?? DEFAULT_PROPS.showNavigation,
    incident: { ...DEFAULT_PROPS.incident, ...props.incident },
    dispatch: { ...DEFAULT_PROPS.dispatch, ...props.dispatch },
    photo: { ...DEFAULT_PROPS.photo, ...props.photo },
    handwrittenNotes: props.handwrittenNotes || DEFAULT_PROPS.handwrittenNotes,
    classification: { ...DEFAULT_PROPS.classification, ...props.classification },
    photoCaption: { ...DEFAULT_PROPS.photoCaption, ...props.photoCaption },
  }
  ...
  const { incident, dispatch, photo, handwrittenNotes } = config;
    ...
    <>
      ...
      {config.showNavigation && (
        <nav className="fixed top-4 left-4 z-50 flex gap-4">
          <Link href="/" className="bg-white/90 px-4 py-2 rounded shadow-lg text-sm font-bold hover:bg-gray-100">
            Clean Version
          </Link>
          <Link
            href="/weathered"
            className="bg-white/90 px-4 py-2 rounded shadow-lg text-sm font-bold border-2 border-blue-600"
          >
            Weathered Version
          </Link>
        </nav>
      ...
      >
        <div className="relative w-full max-w-2xl font-special-elite bg-[#f4f1e8] shadow-2xl shadow-black/60 p-8 md:p-12 text-black/70 rounded-sm weathered-document">
          ...
          <div className="absolute inset-0 bg-[radial-gradient(120%_100%_at_50%_0%,_rgba(0,0,0,0.15),_transparent_60%)]"></div>
          ...
          <header className="relative z-10">
            <h1 className={cn("text-4xl md:text-6xl font-anton tracking-wider text-neutral-800/80 drop-shadow-sm")}>
              {config.incident.title}
            </h1>
            <h2 className={cn("text-3xl md:text-5xl font-anton tracking-wider text-neutral-800/70 drop-shadow-sm")}>
            ...
            >
              <span className={cn("text-2xl font-anton text-red-800/70 tracking-widest drop-shadow-sm")}>
          ...
          <div className="absolute top-[45%] right-4 md:right-8 w-48 h-56 md:w-56 md:h-64 bg-yellow-50 p-3 shadow-xl transform rotate-6 weathered-photo">
            <div className="relative w-full h-full bg-black">
              <Image
                ...
                alt={config.photo.alt || "Photo"}
                fill
                className="object-cover sepia-[0.3] contrast-[0.9] brightness-[0.95]"
              />
              {/* Photo Caption */}
              <PhotoCaption
                labelTop={config.photoCaption.labelTop || "Field Evidence"}
                captionNote={config.photoCaption.captionNote || "Witness account verified"}
                className="bottom-0 left-0 text-xs"
              />
              {/* Photo Caption */}
              <PhotoCaption
                labelTop={config.photoCaption.labelTop}
                captionNote={config.photoCaption.captionNote}
                className="bottom-0 left-0 text-xs"
              />
              {/* Photo Aging Effects */}
