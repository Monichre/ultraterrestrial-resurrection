...
- **Status:** IN PROGRESS — audit + plan landed 2026-08-01 (synced to Linear 2026-08-15, [DMGD-221](https://linear.app/digital-mischief-group/issue/DMGD-221)); **H0 landed** (real commits **`64edf9b5`** disclosure-rag/db "H0 P0s — restore process_for_rag, dedup guard, chunk search", **`25357789`** knowledge-base "H0 archive hygiene — derived zone, dead module surface"). **H1 landed 2026-09-13 except its UNIQUE constraint, which is blocked — see the H1 block below; H2–H5 remain, H2 (the Neon bridge) is next.** **2026-08-16:** OpenAI vector-store upload crash fixed — `file_paths` dict values (trace_map, rag_pipeline) were being passed to `os.path.exists()`, crashing with `stat: path should be string, bytes, os.PathLike or integer, not dict`. Added `isinstance(v, str)` guard in `lib/kb/knowledge_base_service.py:436-438`. This was blocking `--upload` runs from depositing to the vector store.
...
- **✅ Entity resolution (name lookup) — schema live, CLI shipped, review queue loaded. Updated 2026-09-13 21:46 CDT (2026-09-14 02:46 UTC).** Every entity table's only key was an opaque `rec_*`/uuid `id`, so an NER mention could not find the row it names (the Vallée episode: 74 extracted, 0 matched).
  - **Schema — `bf34ce1c`:** [`packages/db/migrations/003_entity_resolution.sql`](packages/db/migrations/003_entity_resolution.sql), **applied to Neon 2026-09-13 23:23 UTC.** `entity_aliases` (774 seeded names; unique per *entity*, not per table, so namesakes coexist and route to review) + `document_entities` extended into the mention log / review queue + `entity_normalize()` as the single normalizer. No `canonical_key`: a computed key only catches exact spellings; aliases cover that and every variant.
  - **CLI — `0ffcec6b`:** [`apps/disclosure-rag/scripts/entity_resolution.py`](apps/disclosure-rag/scripts/entity_resolution.py) via `dy load-mentions | resolve | review | apply`; logic in [`apps/disclosure-rag/lib/entity_resolution/`](apps/disclosure-rag/lib/entity_resolution/). resolve auto-matches only an unambiguous exact alias; code never decides `new`; review commits one decision at a time and cascades same-spelling mentions; apply is the only stage that creates entity rows. load/resolve/apply are dry runs unless `--commit`.
  - **Evidence:** `pytest tests/test_entity_resolution.py tests/test_main.py tests/test_postgres_client.py` → **52 passed** (8 new; DB tests run the real migration on a throwaway local Postgres). Live run: `dy load-mentions --commit` → 113 mentions from 68 pipeline files (fail-verdict chunks excluded); `dy resolve --commit` → **13 matched, 84 needs_review, 16 skipped** (EVIDENCE — no Claims table). Entity tables unchanged (465/40/142). `printf 'q\n' | dy review --table key_figures` rendered the first mention screen and quit with 0 decisions.
  - **Open:** (1) **operator:** work the 84-mention queue with `dy review`, then `dy apply` (dry run) → `dy apply --commit`; (2) merge the 5 duplicate rows the seed surfaced (Däniken, Albert M. Chop, Brian O'Leary, Marc D'Antonio, New Paradigm Institute) — no merge tool exists; (3) only 29 of 113 mentions link to a Neon `documents` row (the rest carry `metadata.kb_path`) until H2 lands; (4) NER emits noise names ("The President", "Media", "Unnamed staffer") — prompt-side fix, not resolver; (5) no LLM tiebreak tier (completions chain still blocked); (6) wiring `dy <url>` to queue mentions automatically is not done — `load-mentions` is a separate step.
...
- **Status:** BUILT, COMMITTED — code-complete, **verified live over stdio 2026-08-09 and re-verified 2026-08-15** (10 results; 168,636-char fetch; duplicate-`file_id` defect observed in top-3); **not reachable from Claude Code** ([DMGD-218](https://linear.app/digital-mischief-group/issue/DMGD-218), Linear state **Backlog**). The whole package landed in `51e63500` (2026-08-14 "here goes") — the old "UNCOMMITTED / absent from a fresh clone" wording is **stale**. Operations & fidelity guide: `packages/openai-vector-store-mcp/USAGE.md` (corpus-integrity workflows A–F, tool contracts, fixes, additions backlog, verification log). Remaining: root `.mcp.json` registration (deliberate action item — do after the §6 fixes), `fastmcp>=3` floor, test suite, sse/http verification.
