...
### 5.3 Research Session Integration (Future)

When the Research Canvas panel is integrated (Feature #5 in FEATURES.md), investigation sessions should scaffold around this methodology:

- Session opening: select primary classification target (event type, CE class)
- Evidence board: attach documents with source tier labels
- Corroboration tracker: visualize the corroboration matrix score as evidence is added
- Pattern flags: auto-highlight geographic/temporal cluster membership from existing entity graph
