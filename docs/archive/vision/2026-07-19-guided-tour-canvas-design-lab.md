...
**Paused:** 2026-07-19 — no finalize, no cleanup, no `DESIGN_PLAN.md` yet.

When resuming:

1. Confirm F (or iterate: node height, edge-label noise, edges-only vs record+edges).
2. Final preview at `/__design_preview` (or current `/design-lab` flow).
3. On confirm: cleanup temp lab per design-lab skill → write `DESIGN_PLAN.md` + update `DESIGN_MEMORY.md` / `docs/vision/UI_INSPIRATION.md` with Polyphonic + apossible entries.
4. Optionally open a **second** lab for Investigate (Deep Well) or Empty+Observer — out of scope for F.
...
- `docs/vision/ultraterrestrial-nuclear-shadow-xyflow/` — guided-tour vision package  
- `docs/vision/ultraterrestrial-nuclear-shadow-xyflow/src/features/guided-tours/nuclear-shadow/nuclear-shadow.definition.ts` — tour definition source  
