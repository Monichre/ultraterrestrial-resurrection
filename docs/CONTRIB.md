...
ultraterrestrial-resurrection/          # Monorepo root
  ...
  docs/
    plans/                              # Strategic plans, feature specs, TODOs
    agents/                             # Agent-specific configurations
    work_logs/                          # Historical session logs
    prose/                              # Long-form documentation
```
...
| `db:cmd:ask` | `cd packages/db && bun run cmd:ask` | Ask Xata a question |
| `db:cmd:search` | `cd packages/db && bun run cmd:search` | Full-text search |
| `db:cmd:aggregate` | `cd packages/db && bun run cmd:aggregate` | Aggregation queries |
| `db:cmd:ufo-credibility` | ... | UFO credibility analysis |
| `db:cmd:ufo-disclosure` | ... | Disclosure timeline |
| `db:cmd:ufo-timeline` | ... | UFO event timeline |
| `db:cmd:ufo-geographic` | ... | Geographic analysis |
| `db:cmd:multi-table` | ... | Cross-table queries |
| `db:cmd:demo` | ... | Demo Xata commands |
...
3. Check `docs/plans/2026-03-29-roundtable-unified-action-plan.md` for the current hardening plan
